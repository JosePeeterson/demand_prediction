...
class TimeSeriesTrain(Dataset):
    ...
    def _sort_by_timestamp(
        ...
        if isinstance(self._data[time_name].iloc[0], int):
            self._data[time_name] = pd.to_datetime(self._data[time_name] - 1, unit = 'h')
            self._freq = 'h'
    ...
    def _standardize(
        ...
        covariate_names = ['target', self._grouping_name, self._time_name,'demand']
    ...
    def _pad_ts(
        ...
        covariate_names = ['target', 'prev_target', self._grouping_name, self._time_name,'demand']
    ...
    def _sample_missing_prev_tgts(
        ...
        #print('\n\n\n df[self._grouping_name] \n\n\n',df[self._grouping_name])
        category = 'dummy_cat' #df[self._grouping_name].iloc[0]
...
class TimeSeriesTest(TimeSeriesTrain):
    ...
    def _preprocess_new_test_data(
        ...
        covariate_names = [self._grouping_name, self._time_name, 'demand']
        covariate_mask = [False if col_name in covariate_names else True for col_name in self._data.columns]
        if self._data.shape[0] > 0:
            self._data.loc[:, covariate_mask] = self._scaler.transform(self._data.loc[:, covariate_mask].astype('float'))
    ...
    def next_batch(
        ...
        print('\nself._prepped_data[df_start_idx:df_stop_idx]\n',self._prepped_data[df_start_idx:df_stop_idx])
        # sample missing 'targets' from current model parameters (for 'prev_targets')
        batch_data = [
